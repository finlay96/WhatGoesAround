...
from exploration.cotracker_src.cotracker3_online import CoTrackerThreeBase, posenc
...
class CoTrackerThreeOfflineCustom(CoTrackerThreeBase):
    def __init__(self, **args):
        super(CoTrackerThreeOfflineCustom, self).__init__(**args)
    ...
    def forward(
        ...
        #     )
        #     fmaps_pyramid.append(fmaps)
        fmaps_pyramid = unet_fmaps
        T = fmaps_pyramid[0].shape[1]
        ...
        for i in range(self.corr_levels):
            track_feat, track_feat_support = self.get_track_feat(
                ...
                queried_frames,
                # queried_coords / 2 ** i,
                queried_coords, # TODO NEED TO SCALE THE COORDS ACCORDINGLY
...
if __name__ == "__main__":
    ...
    cotracker = CoTrackerThreeOfflineCustom().to(device=device)
    cotracker(torch.randn(2, 5, 3, 256, 256).to(device), torch.randn(2, 10, 3).to(device))
