...
from exploration.argus_src.mast3r.model import AsymmetricMASt3R
from exploration.argus_src.mast3r.cloud_opt.sparse_ga import sparse_global_alignment
from exploration.argus_src.mast3r.dust3r.utils.image import load_images
from exploration.argus_src.mast3r.image_pairs import make_pairs
from exploration.argus_src.mast3r.retrieval.processor import Retriever
from exploration.argus_src.mast3r.demo import _convert_scene_output_to_glb
from exploration.argus_src.mast3r.dust3r.utils.device import to_numpy
from exploration.argus_src.mast3r.cloud_opt.tsdf_optimizer import TSDFPostProcess
...
def get_camera_params_from_frames(frames: torch.Tensor,
                                  ...
                                  model=None, unnorm=True) -> Tuple[torch.tensor, torch.tensor, float]:
    '''
    frames: torch.Tensor, in T*C*H*W format, normalized to [-1, 1] or [0, 1] if unnorm=False
    ...
    cache_dir = "/home/finlay/part_time_PHD_stuff/tracking/WhatGoesAround/runners/estimating_poses/cache/tmpq3e9lyr9_cache"
    for i in range(25):
        img_paths.append(os.path.join(cache_dir, f"frame_{i}.png"))
