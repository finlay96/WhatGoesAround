...
from .src import generate_mask_batch, pers2equi_batch, partial360_to_pers
...
def sample_svd(args, accelerator, pipeline,
               ...
               eq_frames=None,
               eq_mask=None,
               roll: np.ndarray = None, pitch: np.ndarray = None, yaw: np.ndarray = None,
               # in radians, numpy array of length T
    ...
    if equirectangular_input:
        ...
        conditional_video_pers = equi2pers(conditional_video.to(torch.float32), fov_x=fov_x,
                                           ...
                                           rots=rots, z_down=True).to(
            weight_dtype)  # in default height and width, will be sent to the CLIP

        if not post_rotation:
            conditional_video_equi = torch.where(mask == 1, conditional_video,
                                                 torch.randn_like(conditional_video,
                                                                  device=accelerator.device) * noise_conditioning_strength \
                                                     if noise_conditioning else -torch.ones_like(conditional_video,
                                                                                                 device=accelerator.device))
        else:
            zeros = torch.zeros(T, device=accelerator.device)
            conditional_video_equi = pers2equi_batch(conditional_video_pers, fov_x=fov_x, height=height, width=width,
                                                     device=accelerator.device,
        ...
        if eq_frames is not None:
            conditional_video_equi = eq_frames.to(weight_dtype)
            mask = eq_mask
        else:
            # TODO use own mapper logic for this
        ...
        if noise_conditioning:
            conditional_video_equi = torch.where(mask == 1, conditional_video,
                                                 torch.randn_like(conditional_video, device=accelerator.device))
        conditional_video_equi = conditional_video_equi.unsqueeze(0)
        hw_ratio = conditional_video_pers.shape[-2] / conditional_video_pers.shape[-1]

    if narrow:
        conditional_video_equi = conditional_video_equi[
            :, :, :, (height - height_narrow) // 2:(height + height_narrow) // 2, (width - width_narrow) // 2:(
                                                                                                                      width + width_narrow) // 2]
        mask = mask[:, :, (height - height_narrow) // 2:(height + height_narrow) // 2, (width - width_narrow) // 2:(
                                                                                                                           width + width_narrow) // 2]
        if equirectangular_input:
            conditional_video = conditional_video[
                :, :, (height - height_narrow) // 2:(height + height_narrow) // 2, (width - width_narrow) // 2:(
                                                                                                                       width + width_narrow) // 2]
    ...
    with torch.autocast(str(accelerator.device).replace(":0", ""), enabled=accelerator.mixed_precision != 'no',
        ...
        num_frames_batch = args.num_frames_batch if (
                hasattr(args, 'num_frames_batch') and args.num_frames_batch is not None) else args.num_frames
        ...
        while num_frames_remaining > 0:
            ...
            conditional_video_input = conditional_video_input + torch.randn_like(conditional_video_input,
                                                                                 device=accelerator.device) * noise_aug_strength * mask_this
            ...
            if blend_frames != 0:  # save current generation results into a file
                generated_frames_this = pipeline.decode_latents(generated_latents_this, num_frames_to_process,
                                                                decode_chunk_size=decode_chunk_size,
                                                                extended_decoding=extended_decoding,
                                                                blend_decoding_ratio=blend_decoding_ratio)
            ...
            if generated_latents is None:
                ...
                blend_weight = torch.linspace(1, 0, blend_frames, device=accelerator.device,
                ...
                generated_latents = torch.cat([generated_latents[:, :-blend_frames],
                                               blend_weight * generated_latents[:, -blend_frames:] + (
                                                       1 - blend_weight) * generated_latents_this[:, :blend_frames],
        ...
        generated_frames = pipeline.decode_latents(generated_latents, args.num_frames,
                                                   decode_chunk_size=decode_chunk_size,
                                                   extended_decoding=extended_decoding,
                                                   blend_decoding_ratio=blend_decoding_ratio)
    ...
    if post_rotation:
        rots = [{"roll": -roll[i], "pitch": -pitch[i], "yaw": -yaw[i]} for i in range(args.num_frames)]
        generated_frames = equi2equi(generated_frames.permute(0, 2, 1, 3, 4).flatten(0, 1), rots=rots,
                                     z_down=True).permute(1, 0, 2, 3).view(generated_frames.shape)
        conditional_video_equi = equi2equi(conditional_video_equi.flatten(0, 1), rots=rots, z_down=True).view(
            conditional_video_equi.shape)

    # save the input video
    save_path = out_file_path.replace(ext, f"_input_fov{fov_x:.0f}_hw{hw_ratio:.2f}.mp4")
    conditional_video_equi = ((conditional_video_equi.clamp(-1, 1) + 1) * 127.5).cpu().to(torch.float32).numpy().astype(
        np.uint8)
    ...
    if equirectangular_input:
        conditional_video = ((conditional_video.clamp(-1, 1) + 1) * 127.5).cpu().to(torch.float32).numpy().astype(
            np.uint8)
        conditional_video = conditional_video.transpose(0, 2, 3, 1)  # (T, H, W, C)
        concatenated_video = np.concatenate([conditional_video_equi, generated_frames, conditional_video],
                                            axis=1)  # (T, 3H, W, C)
    ...
    if narrow:  # transform the narrow video into perspective format in orignal fov
        ...
        partial360_to_pers(return_file_path, save_path, fov_x=fov_x, hw_ratio=hw_ratio, width=args.width,
                           height=args.height)
